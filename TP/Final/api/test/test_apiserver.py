...
from eth_account.messages import SignableMessage, encode_defunct  # type: ignore
...
creators_list_schema = {
    "type": "object",
    "properties": {"creators": {"type": "array", "items": {"type": "string"}}},
    "required": ["creators"],
}
...
def post_create(account: Account, call_id, closing_time: Union[datetime, str]):
    ...
    return requests.post(
        url("create"),
        json={"callId": call_id, "signature": signature, "closingTime": closing_time},
        timeout=10,
    )
...
def post_register(address, signature):
    ...
    return requests.post(
        url("register"), json={"address": address, "signature": signature}, timeout=10
    )
...
def post_register_proposal(call_id, proposal):
    ...
    return requests.post(
        ...
        json={"callId": call_id, "proposal": proposal},
        timeout=10,
    )
...
def test_create_invalid_time_format() -> None:
    ...
    for closing_time in invalid:
        ...
        assert APPLICATION_JSON in response.headers["Content-type"]
        validate(instance=response.json(), schema=message_schema)
        assert response.json()["message"].startswith(messages.INVALID_TIME_FORMAT)
        assert response.status_code == 400
...
def test_register_proposal_invalid_mimetype() -> None:
    ...
    for call_id in calls:
        ...
        response = requests.post(
            url("register-proposal"),
            data={"callId": call_id, "proposal": proposal},
            timeout=10,
        )
        assert APPLICATION_JSON in response.headers["Content-type"]
        validate(instance=response.json(), schema=message_schema)
...
def test_get_all_creators_success():
    """Prueba que el endpoint /creators devuelve la lista de creadores correctamente."""
    response = requests.get(url("creators"), timeout=1000)

    # Verificar headers y status code
    assert APPLICATION_JSON in response.headers["Content-type"]
    assert response.status_code == 200

    # Validar estructura de la respuesta
    validate(instance=response.json(), schema=creators_list_schema)

    # Verificar que la respuesta contiene una lista de creadores
    data = response.json()
    assert "creators" in data
    assert isinstance(data["creators"], list)


def test_get_all_calls_success():
    """Prueba que el endpoint /calls devuelve la lista de calls correctamente."""
    response = requests.get(url("calls"), timeout=1000)
    ...
    # Si hay calls, validar estructura
    if len(data) > 0:
        validate(instance=data, schema=calls_list_schema)


def test_get_pendings() -> None:
    """Prueba que /pendings devuelve las direcciones pendientes de autorización."""
    response = requests.get(url("pendings"), timeout=1000)
    assert APPLICATION_JSON in response.headers["Content-type"]
    assert response.status_code == 200
    data = response.json()
    assert "pending" in data
    assert isinstance(data["pending"], list)
    # No debe haber cuentas ya autorizadas en pending
    for account in accounts:
        assert account.address not in data["pending"]
