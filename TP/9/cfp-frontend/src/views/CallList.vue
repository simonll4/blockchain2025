      ...
      <v-col v-for="call in calls" :key="call.callId" cols="12" md="6" lg="4">
        <v-card class="pa-4" elevation="3">
          <v-card-title class="text-h6">
            Llamado: {{ shortenAddress(call.callId) }}
          </v-card-title>
          <v-card-text>
            <div class="mb-2">
              <strong>Creador:</strong>
              <v-chip class="ma-1" color="blue-grey lighten-4" text-color="black">
          ...
          <v-card-actions class="justify-end">
            <v-btn :to="`/register/${call.callId}`" color="primary" variant="flat" class="me-2">
              Registrar
            </v-btn>
            <v-btn :to="`/verify/${call.callId}`" color="secondary" variant="flat">
              Verificar
            </v-btn>
          </v-card-actions>
        </v-card>
